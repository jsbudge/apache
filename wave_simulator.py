...
from simulib.simulation_functions import getElevation, llh2enu, db, enu2llh, azelToVec, genPulse
...
def reloadWaveforms(wave_mdl, pulse_data, nr, fft_len, tc_d, rps, cpi_len, bwidth):
    waves = wave_mdl.full_forward(pulse_data, tc_d, nr, bwidth)
...
if __name__ == '__main__':
    ...
    grot = np.array([0., 0., 0.])
    rpi = ApachePlatform(e, n, u, r, p, y, t, dep_angle=req_dep_ang / DTR,
                         gimbal=np.array([gim_pan, gim_el]).T, gimbal_rotations=grot, gimbal_offset=goff, fs=2e9,
                         params=wave_config['apache_params'])
    rpi.fc = 9.6e9
    rpi.bwidth = 400e6
