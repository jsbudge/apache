...
class SwiGLU(nn.Module):
    def __init__(self, in_features, hidden_features, out_features):
        super().__init__()
        # The first linear layer typically projects to a wider dimension (e.g., 8/3 * in_features)
        # and its output is chunked into two parts: a gate and a value.
        self.fc1 = nn.Linear(in_features, hidden_features * 2)
        self.fc2 = nn.Linear(hidden_features, out_features)
        self.silu = nn.SiLU() # PyTorch's implementation of Swish/SiLU

    def forward(self, x):
        # Apply the first linear layer
        x = self.fc1(x)
        # Split the tensor into two halves along the feature dimension (dim=-1)
        x_gate, x_value = torch.chunk(x, 2, dim=-1)
        # Apply SiLU activation to the gate and multiply element-wise with the value
        x = self.silu(x_gate) * x_value
        # Apply the second linear layer to project back to the output dimension
        x = self.fc2(x)
        return x
...
class MultiHeadLocationAwareAttention(nn.Module):
    ...
    def forward(self, query: Tensor, value: Tensor, last_attn: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        query = query.contiguous().view(-1, seq_len, self.dim)
...
class RBFLayer(nn.Module):
    ...
    def __init__(self,
        ...
        assert not normalization or normalization
