...
if __name__ == '__main__':
    ...
    tmeans = [np.zeros((2, 8192)) for r in root_dirs if 'target_' in r]
    ...
    for t_dir in os.listdir(root_path):
        if 'target' in t_dir:
            ...
            for f in tqdm(files):
                ...
                tmeans[label] += sample.cpu().data.numpy() / len(files)

    for idx, m in enumerate(tmeans):
        tmeans[idx] = embedding(torch.tensor(m, dtype=torch.float32).unsqueeze(0).to(embedding.device)).cpu().data.numpy()
